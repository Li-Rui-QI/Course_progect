...
def extract(path, data_file, requested=None):
    if path is None or type(path) != str or len(path) == 0:
        import urllib
        from smb.SMBHandler import SMBHandler
        opener = urllib.request.build_opener(SMBHandler)
        print('Connecting to remote...')
        file = opener.open('smb://guest:Globus-M@172.16.12.127/Data/sht%d.SHT' % data_file)
    else:
        file = open('%s/sht%d.SHT' % (path, data_file), 'rb')
